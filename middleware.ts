...
//   { path: '/dashboard/admins', roles: ['superAdmin'] },
//   { path: '/dashboard/moderators', roles: ['admin', 'superAdmin'] },
//   { path: '/dashboard/users', roles: ['admin', 'superAdmin'] },
//   { path: '/dashboard/posts', roles: ['moderator', 'superAdmin'] },
//   { path: '/dashboard/posts/create', roles: ['moderator', 'superAdmin'] },
// ];
...
export async function middleware(request: NextRequest) {
  ...
  if (!token) {
    return new NextResponse(JSON.stringify({ error: 'Missing authentication token' }), { 
      status: 401, 
  ...
  if (isTokenExpired(token)) {
    const response = new NextResponse(JSON.stringify({ error: 'Token expired, please sign in again' }), { 
      status: 401, 
      headers: { 'Content-Type': 'application/json' }
    });
    response.cookies.delete('auth_token');
  ...
  try {
    ...
    if (requiredRoles && !requiredRoles.includes(userRole)) {
      return new NextResponse(JSON.stringify({ error: 'You do not have permission to access this resource' }), { 
        status: 403, 
        headers: { 'Content-Type': 'application/json' }
      });
    }
  } catch {
    const response = new NextResponse(JSON.stringify({ error: 'Invalid authentication token' }), { 
      status: 401, 
      headers: { 'Content-Type': 'application/json' }
    });
    response.cookies.delete('auth_token');
